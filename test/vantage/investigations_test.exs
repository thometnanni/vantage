defmodule Vantage.InvestigationsTest do
  ...
  describe "investigations" do
    ...
    test "create_investigation/1 with valid data creates a investigation" do
      ...
      assert {:ok, %Investigation{} = investigation} =
               Investigations.create_investigation(valid_attrs)
    ...
    test "update_investigation/2 with valid data updates the investigation" do
      ...
      update_attrs = %{
        ...
        time: ~U[2024-12-05 10:13:00.000000Z],
      ...
      assert {:ok, %Investigation{} = investigation} =
               Investigations.update_investigation(investigation, update_attrs)
    ...
    test "update_investigation/2 with invalid data returns error changeset" do
      ...
      assert {:error, %Ecto.Changeset{}} =
               Investigations.update_investigation(investigation, @invalid_attrs)
    ...
    test "delete_investigation/1 deletes the investigation" do
      ...
      assert_raise Ecto.NoResultsError, fn ->
        Investigations.get_investigation!(investigation.id)
      end
    end
  ...
  describe "investigation_collaborators" do
    ...
    test "get_investigation_collaborator!/1 returns the investigation_collaborator with given id" do
      ...
      assert Investigations.get_investigation_collaborator!(investigation_collaborator.id) ==
    ...
    test "create_investigation_collaborator/1 with valid data creates a investigation_collaborator" do
      valid_attrs = %{role: "writer"}

      assert {:ok, %InvestigationCollaborator{} = investigation_collaborator} =
               Investigations.create_investigation_collaborator(valid_attrs)

      assert investigation_collaborator.role == "writer"
    end

    test "create_investigation_collaborator/1 with invalid data returns error changeset" do
      assert {:error, %Ecto.Changeset{}} =
               Investigations.create_investigation_collaborator(@invalid_attrs)
    end

    test "update_investigation_collaborator/2 with valid data updates the investigation_collaborator" do
      investigation_collaborator = investigation_collaborator_fixture()
      update_attrs = %{role: "reader"}

      assert {:ok, %InvestigationCollaborator{} = investigation_collaborator} =
               Investigations.update_investigation_collaborator(
                 investigation_collaborator,
                 update_attrs
               )

      assert investigation_collaborator.role == "reader"
    end
    ...
    test "update_investigation_collaborator/2 with invalid data returns error changeset" do
      ...
      assert investigation_collaborator ==
               Investigations.get_investigation_collaborator!(investigation_collaborator.id)
    end

    test "delete_investigation_collaborator/1 deletes the investigation_collaborator" do
      investigation_collaborator = investigation_collaborator_fixture()

      assert {:ok, %InvestigationCollaborator{}} =
               Investigations.delete_investigation_collaborator(investigation_collaborator)
    ...
    test "change_investigation_collaborator/1 returns a investigation_collaborator changeset" do
      ...
      assert %Ecto.Changeset{} =
               Investigations.change_investigation_collaborator(investigation_collaborator)
    end
  end
end
